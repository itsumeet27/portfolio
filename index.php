<?php include ('includes/header.php'); ?>
  <!-- Banner -->
  <div class="banner row animated fadeIn slow">
    <div class="col-md-6 details">
      <h6 class="text-justify h6-responsive">Hello everyone, I am!</h6>
      <h1 class="text-justify h1-responsive">Sumeet Sharma</h1>
      <hr class="title">
      <p class="text-justify lead">Graduate Engineering Trainee | Web Developer | WordPress Developer</p>
      <p class="lead text-justify">
        A hard working individual with the intent of delivering products and services in a timely and most
        efficient manner.
      </p>
      <div class="social-icons">
        <a href="https://www.facebook.com/profile.php?id=100007900753991" target="_blank">
          <i class="fab fa-facebook-f" title="Facebook"></i>
        </a>
        <a href="https://twitter.com/sumeet_270296" target="_blank">
          <i class="fab fa-twitter" title="Twitter"></i>
        </a>
        <a href="https://linkedin.com/in/itsumeet" target="_blank">
          <i class="fab fa-linkedin" title="Linkedin"></i>
        </a>
        <a href="https://instagram.com/sumit270296" target="_blank">
          <i class="fab fa-instagram" title="Instagram"></i>
        </a>
        <a href="https://github.com/sumit270296" target="_blank">
          <i class="fab fa-github mr-2" title="GitHub"></i>
        </a>
      </div>
      <a href="" class="btn btn-portfolio">Portfolio</a>
      <a href="" class="btn btn-contact">Hire Me</a>
    </div>
    <div class="col-md-6 razorBlade">
      <div class="row image-right">
        <div class="col-md-6 image">
          <img src="img/sumeet.jpg">
        </div>
      </div>
    </div>
  </div>
  <!-- Banner -->

  <!--Main layout-->
  <main>
    <!--Section: About-->
    <div class="container">
      <section id="about">
        <div class="section-title p-4 text-center">
          <h1 class="h1-responsive">About</h1>
          <hr class="section">
        </div>
        <!--Grid row-->
        <div class="row">
          <!--Grid column-->
          <div class="col-md-6 mb-4 animated fadeInLeft slow delay-1s">
            <img src="img/sumit.jpg" class="img-fluid z-depth-1-half" alt="">
          </div>
          <!--Grid column-->

          <!--Grid column-->
          <div class="col-md-6 mb-4 animated fadeInRight slow delay-1s">
            <p class="text-justify lead">
              Experienced Web Developer with a demonstrated history of working in the higher education industry. Skilled
              in
              Web framework such as Django, and PHP, WordPress, Java, HTML, CSS, Javascript, Bootstrap, Responsive Web
              Design,
              and Leadership. Strong engineering professional with an Engineer’s Degree focused in Information
              Technology from
              St. Francis Institute Of Technology.
            </p>
            <div class="technologies">
              <i class="fab fa-html5" title="HTML5"></i>
              <i class="fab fa-css3" title="CSS3"></i>
              <i class="fab fa-js" title="JavaScript"></i>
              <i class="fab fa-php" title="PHP"></i>
              <i class="fab fa-bootstrap" title="Bootstrap"></i>
              <i class="fab fa-mdb" title="Material Design Bootstrap"></i>
              <i class="fab fa-wordpress" title="WordPress"></i>
              <i class="fab fa-angular" title="Angular"></i>
              <i class="fas fa-code-branch" title="GitHub"></i>

            </div>
            <a href="" class="btn btn-contact mt-4 ml-0">Read More</a>
          </div>
          <!--Grid column-->
        </div>
        <!--Grid row-->
      </section>
    </div>

    <!--Section: Education-->
    <div class="education">
      <!--Section: Main info-->
      <section class="wow fadeIn">
        <div class="section-title p-4 text-center">
          <h1 class="h1-responsive">Education</h1>
          <hr class="section">
        </div>
        <div class="timeline">
          <div class="timeline-container left animated slideInLeft slow delay-1s">
            <div class="content">
              <h2 class="h2-responsive text-left">2014-2018</h2>
              <h4 class="h4-responsive text-justify">Bachelor Of Engineering (Information Technology)</h4>
              <p class="text-justify">
                St. Francis Institute of Technology, Borivali (W), Mumbai
              </p>
              <hr>
              <h5 class="h5-responsive float-left">Result (CGPA): </h5><span class="float-right">7.98/10</span>
            </div>
          </div>
          <div class="timeline-container right animated slideInRight slow delay-2s">
            <div class="content">
              <h2 class="h2-responsive text-left">2012-2014</h2>
              <h4 class="h4-responsive text-justify">Higher Secondary Certificate (HSC)</h4>
              <p class="text-justify">
                Thakur College of Science and Commerce, Kandivali (E), Mumbai
              </p>
              <hr>
              <h5 class="h5-responsive float-left">Result: </h5><span class="float-right">80.62%</span>
            </div>
          </div>
          <div class="timeline-container left animated slideInLeft slow delay-3s">
            <div class="content">
              <h2 class="h2-responsive text-left">2011-2012</h2>
              <h4 class="h4-responsive text-justify">Secondary School Certificate (HSC)</h4>
              <p class="text-justify">
                Cosmopolitan High School, Mira Road (E), Thane
              </p>
              <hr>
              <h5 class="h5-responsive float-left">Result: </h5><span class="float-right">86.91%</span>
            </div>
          </div>
        </div>
      </section>
    </div>

    <!--Section: Portfolio-->
    <div class="container-fluid animated fadeIn slow">
      <!--Section: Main features & Quick Start-->
      <section class="portfolio" id="portfolio">
        <div class="section-title p-4 text-center">
          <h1 class="h1-responsive">Portfolio</h1>
          <hr class="section">
        </div>
        <div class="row">
          <div class="col-md-4 animated fadeInLeft delay-3s slow">
            <!-- Card -->
            <div class="card card-image m-2">
              <!-- Content -->
              <img src="img/portfolio/amittradersindia.png" />
              <div class="card-overlay p-4">
                <h5 class="black-text"><i class="fas fa-shopping-cart"></i> E-Commerce</h5>
                <h3 class="card-title pt-2"><strong>Amit Traders India</strong></h3>
                <p class="text-justify">Manufacturers and Suppliers of Leather goods and Corporate gift articles </p>
                <a class="btn btn-outline-black" href="https://amittradersindia.com"><i class="fas fa-clone left"></i>
                View project</a>
              </div>
            </div>
            <!-- Card -->
          </div>
          <div class="col-md-4 animated fadeInUp delay-3s slow">
            <!-- Card -->
            <div class="card card-image m-2">
              <!-- Content -->
              <img src="img/portfolio/khadipremiumcosmetics.png" />
              <div class="card-overlay p-4">
                <h5 class="black-text"><i class="fas fa-shopping-cart"></i> E-Commerce</h5>
                <h3 class="card-title pt-2"><strong>Khadi Premium Cosmetics</strong></h3>
                <p class="text-justify">A wide range of natural and herbal cosmetic
                products to take care for your skin, hair and body. </p>
                <a class="btn btn-outline-black" href="https://khadipremiumcosmetics.com"><i
                  class="fas fa-clone left"></i>
                View project</a>
              </div>
            </div>
            <!-- Card -->
          </div>
          <div class="col-md-4 animated fadeInRight delay-3s slow">
            <!-- Card -->
            <div class="card card-image m-2">
              <!-- Content -->
              <img src="img/portfolio/filaydent.png" />
              <div class="card-overlay p-4">
                <h5 class="black-text"><i class="fas fa-shopping-cart"></i> E-Commerce</h5>
                <h3 class="card-title pt-2"><strong>FilayDent</strong></h3>
                <p class="text-justify">An E-Commerce website for instruments used in dental surgery </p>
                <a class="btn btn-outline-black" href="https://filaydent.com"><i class="fas fa-clone left"></i>
                View project</a>
              </div>
            </div>
            <!-- Card -->
          </div>
        </div>
      </section>
    </div>

    <!-- Section: Work Experience -->
    <div id="experience">
      <div class="experience-overlay experience">
        <div class="section-title p-4 text-center">
          <h1 class="h1-responsive">Work Experience</h1>
          <hr class="section">
        </div>
        <div class="timeline">
          <div class="timeline-container left animated slideInLeft slow delay-1s">
            <div class="content">
              <h6 class="h6-responsive text-justify">Graduate Engineering Trainee</h6>
              <h4 class="h4-responsive text-left">Larsen and Toubro Infotech, Airoli (E), Mumbai</h4>
              <p class="text-justify">
                <ul>
                  <li>Practice on SOA (Service Oriented Architecture) and OSB (Oracle Service Bus) for
                    developing services
                  </li>
                  <li>APWF Support for providing support on a project based on I/PM (Imaging and Process
                    Management) Technology for incidents occurring due to faults in email processed by the
                    server
                  </li>
                </ul>
              </p>
              <hr>
              <span class="float-right"><i>December 2018 - Current</i></span>
            </div>
          </div>
          <div class="timeline-container right animated slideInRight slow delay-2s">
            <div class="content">
              <h6 class="h6-responsive text-justify">Web Development Manager</h6>
              <h4 class="h4-responsive text-left">Luka Enterprises, Andheri (W), Mumbai</h4>
              <p class="text-justify">
                <ul>
                  <li>Developing an E-Commerce website</li>
                  <li>Managing a website and working on the features</li>
                  <li>Adding features and functionalities to the website</li>
                  <li>Working on the SEO of the website</li>
                </ul>
              </p>
              <hr>
              <span class="float-right"><i>August 2018 – December 2018</i></span>
            </div>
          </div>
          <div class="timeline-container left animated slideInLeft slow delay-3s">
            <div class="content">
              <h6 class="h6-responsive text-justify">Web Developer</h6>
              <h4 class="h4-responsive text-left">Indian Institute of Digital Education (IIDE), Andheri (W), Mumbai</h4>
              <p class="text-justify">
                <ul>
                  <li>Managed and Developed website on WordPress</li>
                  <li>Have hands on experience on DIVI theme and builder</li>
                  <li>Learned how to maintain SEO on the WordPress website using a plugin called as Yoast SEO</li>
                </ul>
              </p>
              <hr>
              <span class="float-right"><i>April 2017 – August 2017</i></span>
            </div>
          </div>
        </div>
      </div>
    </div>

    <!-- Section: Certifications -->
    <div class="container animated fadeIn delay-1s slow">
      <section class="certifications" id="certifications">
        <div class="section-title p-4 text-center">
          <h1 class="h1-responsive">Online Certifications</h1>
          <hr class="section">
        </div>
        <div class="certification table-responsive">
          <table class="table table-bordered">
            <thead>
              <th>Title of Certification</th>
              <th>Certification Authority</th>
              <th>Domain of Certification</th>
            </thead>
            <tbody>
              <tr>
                <td><a href="https://drive.google.com/open?id=1PkDjq50fcTjEd-2IEE3655LCk1e3NJVS" target="_blank">Online Marketing Fundamentals</a></td>
                <td>Google Digital Unlocked</td>
                <td>Digital Marketing</td>
              </tr>
              <tr>
                <td><a href="https://drive.google.com/open?id=1uQ4kx0JvPjb5P3whT_H7YcgEDzWVZX1X" target="_blank">Introduction to R</a></td>
                <td rowspan="3">Datacamp</td>
                <td rowspan="4">Data Analysis and Machine Learning</td>
              </tr>
              <tr>
                <td><a href="https://drive.google.com/open?id=1ldIDHZTiACjMc7cdjujjVpbPWGYEPXl4" target="_blank">Introduction to Python for Data Science</a></td>
              </tr>
              <tr>
                <td><a href="https://drive.google.com/open?id=1sdE12d2YmX4HftN2YR7tzRNZOKAZzJ_0" target="_blank">Building Web Applications in R with Shiny</a></td>
              </tr>
              <tr>
                <td><a href="https://drive.google.com/open?id=1F6e4r31tNH3T3r-4ODQqFz0r6UMw9Asb" target="_blank">Python 101 for Data Science & Machine Learning with Python</a></td>
                <td>Cognitive Class</td>
              </tr>
              <tr>
                <td><a href="https://drive.google.com/open?id=1b5dtci0ESbHMIlWojzf9UurInafLeXGL" target="_blank">Practical PHP: Master the Basics and Code Dynamic Websites</a></td>
                <td rowspan="2">Udemy</td>
                <td rowspan="2">Web Development</td>
              </tr>
              <tr>
                <td><a href="https://drive.google.com/open?id=1lOJNlrs-OOc4UMUgKBNwJalmrtpMXbS_" target="_blank">Learn E-Commerce Website in PHP & MySQL from Scratch</a></td>
              </tr>
              <tr>
                <td>Brain Bench</td>
                <td>Brain Bench</td>
                <td>Java</td>
              </tr>
            </tbody>
          </table>
        </div>
      </section>
    </div>

    <!-- Section: Contact -->
    <div id="contact">
      <div class="contact-overlay contact">
        <div class="section-title p-4 text-center">
          <h1 class="h1-responsive">Contact</h1>
          <hr class="section">
        </div>
        <div class="container">
          <div class="white-text row">
            <div class="col-md-6 p-2 animated fadeInLeft slow delay-2s">
              <h3 class="h3-responsive text-justify p-2">Reach out today!</h3>
              <p class="text-justify p-2">
                <ul class="list-unstyled mb-0 contact-details">
                  <li class="p-2">
                      <i class="px-2 fas fa-map-marker-alt fa-2x"></i>
                      <span class="px-2">302, B-7, Sector-9, Shanti Nagar, Mira Road (E), Thane-401107</span>
                  </li>

                  <li class="p-2"> 
                      <i class="px-2 fas fa-phone mt-4 fa-2x"></i>
                      <span class="px-2">+91 82868 64601</span>
                  </li>

                  <li class="p-2">
                      <i class="px-2 fas fa-envelope mt-4 fa-2x"></i>
                      <span class="px-2"><a href="mailto:sksksharma0@gmail.com" class="white-text">sksksharma0@gmail.com</a></span>
                  </li>
                </ul>
              </p>
              <div class="location">
                <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d1883.0897740917924!2d72.86277540807464!3d19.274556646743605!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3be7b05a0dd5123b%3A0xf9e5543510f58f4f!2sSector%209%2C%20Shanti%20Nagar%2C%20Mira%20Road%2C%20Mira%20Bhayandar%2C%20Maharashtra%20401107!5e0!3m2!1sen!2sin!4v1574403408310!5m2!1sen!2sin" height="300" frameborder="0" style="border:0;width: 100%" allowfullscreen=""></iframe>
              </div>
            </div>
            <div class="col-md-6 p-2 animated fadeInRight slow delay-2s">
              <section class="contact-form">

                <!--Section heading-->
                <h3 class="h3-responsive text-justify p-2">Drop a message!</h3>
                <!--Section description-->
                <p class="text-justify w-responsive">Do you have any questions? Please do not hesitate to contact us directly. Our team will come back to you within
                a matter of hours to help you.</p>

                <div>
                  <form id="contact-form" name="contact-form" action="mail.php" method="POST">
                    <div class="row">
                      <div class="col-md-6">
                        <div class="md-form mb-0">
                          <input type="text" id="name" name="name" class="form-control">
                          <label for="name" class="">Your name</label>
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="md-form mb-0">
                          <input type="text" id="email" name="email" class="form-control">
                          <label for="email" class="">Your email</label>
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-12">
                        <div class="md-form mb-0">
                          <input type="text" id="subject" name="subject" class="form-control">
                          <label for="subject" class="">Subject</label>
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-12">
                        <div class="md-form">
                          <textarea type="text" id="message" name="message" rows="2" class="form-control md-textarea"></textarea>
                          <label for="message">Your message</label>
                        </div>
                      </div>
                    </div>
                  </form>

                  <div class="text-center text-md-left">
                    <a class="btn btn-outline-white" onclick="document.getElementById('contact-form').submit();">Send</a>
                  </div>
                  <div class="status"></div>
                </div>
              </section>
            </div>
          </div>
        </div>
      </div>
    </div>
  </main>
  <!--Main layout-->
<?php include ('includes/footer.php'); ?>